...
struct StoredKeys<Value> {
    ...
    static var refreshToken: StoredKeys<String> {
        return StoredKeys<String>(Keyname.refreshToken)
    }
    
    static var loggedUserDetails: StoredKeys<User> {
        return StoredKeys<User>(Keyname.loggedUserDetails)
    }
}
